...
const validationSchema = Yup.object({
    ...
    age: Yup.number()
      ...
      .min(5, 'Age must be between 5 and 90')
      .max(90, 'Age must be between 5 and 90'),
...
export const AddModal: React.FC<AddModalProps> = ({ isOpen, onClose, initialValues, refetchUserProfiles }) => {
  ...
  const renderField = ({ name, label, type, options }: any) => (
    <div key={name} className="mb-3">
      ...
      {type === 'select' ? (
        <Field as="select" name={name} className="mt-1 block w-full border-gray-300 border-[1px] px-3 lg:py-2 py-1 rounded-md shadow-sm">
        ...
        <Field name={name} type={type} className="mt-1 block w-full border-gray-300 border-[1px] lg:py-2 py-1 px-3 rounded-md shadow-sm" />
      )}
        ...
        {({ isSubmitting }) => (
          <Form>
            ...
            <div className="flex justify-end gap-2 py-4">
              <button type="button" className="bg-white border-[1px] border-[#001F56] text-[#001F56] px-4 lg:py-2 py-1 rounded-lg" onClick={onClose}>Cancel</button>
              <button type="submit" className="bg-[#001F56] text-white px-4 lg:py-2 py-1 rounded-lg" disabled={isSubmitting}>
